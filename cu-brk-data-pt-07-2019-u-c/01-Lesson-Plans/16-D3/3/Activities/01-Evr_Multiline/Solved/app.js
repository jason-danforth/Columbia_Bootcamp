...
d3.csv("donuts.csv", function(error, donutData) {
  ...
  var yLinearScale = d3.scaleLinear().range([height, 0]); //Reverse mapping b/c Y=0 is at the top of the window
  ...
  var line1 = d3.line() //Don't need to pass arg, will rely on bound data specified later
  ...
  chartGroup //Either of these methods work. Just different options for binding the data
    .data([donutData])
    .append("path")
    .attr("d", line1)
    .classed("line green", true);
  ...
  chartGroup
    .data([donutData]) //Ensure that data is cast as an array and not an object
    .append("path")
    .attr("d", line2)
